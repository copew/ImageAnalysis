...
%[ 593987,619857, 619872, 619905,  619905603288, 593987, 619857, 619872, 619905, 625951, 603292, 603298, 603288, 619857, 619872,];
...
image_list = [603288, 603292, 625958];

% done[603283, 619799, 619857, 619872, 593960,593971, 594006, 602915, 602942, 602976, 602994, 603253, 603269, 603271,  603298, 603956, 619905];

% to check and redo 
% 603269,602976,602942,602915,593960,603283,603271, 603253, 602994,593971,594006,62558,619799,603956,

%done
% 607166,603919,597786,625887,625951,626047,626172,625936,625333,619837,619538,619942


%need to rerun
%607166,603919,603269,602976,602942,602915,593960,597786,603283,603271, 603253, 602994,625887,625951,626047,593971,594006,626172,625936,62558,625333,619837,619799,619538,603956,619942


% done
% 594035,594044,594051,594060,594080,594088,594095,594105, 604089,604098,604083,607165,603944,604078,604057,604071,604077,603911,604105,603925,603922,603262,603263,603257,603006,603245,603007,603055,602983,
% 602971,602962,602952,602958,602966, 602945,602951,602935,602927,602921,593708,593978,597777,597768,597794,597800,597812,597820,597830,597838,599803,599797,599831,599850,599813,603279,604094, 603952, 603952, 594116,594110,595806,594137, 625946,
% 626018,626102,626103,626160, 593987,594000,594017,625865,626171,625876,,625895,625908,625911,619953,625916,626166,625923,625930,619868,619556,619896,619937,619926
% 619877,619539,619460,619954,619465,619470,619476,619483,619489,619503,619508,619496,619515,619527,625891,625322,625338,619877,619539,619460,619954,619465,619470,619476,619483,619489,619503,619508,619496,619515,619527,625891,625322
% 625338,619922,619932,619916,619911,619533,619852,619863,619859,619842,619832,619847,619583,619815,619803,619793,619579,619571,619550,603928,603941,603904,604061,604066,604055,604047,603987,604045,603976,603982,603963,603970,603966,619678,619884,619809,
...
for image = 1:size(image_list, 2)
    ...
    % this path needs changing as it was a bit messed up
    load([src_dir '2/' num2str(image_filenumber) '/' num2str(image_filenumber) '_lymphocyte_polygon20.mat']);
...
%     
    csvwrite([output_dir '/outputfiles_tum_lymph/' num2str(image_filenumber) '/' num2str(image_filenumber) '_centroidin_count20.csv'], t_l_centroid_count_cell);
    csvwrite([ '/Users/cope01/Documents/OneDrive - University Of Cambridge/Documents/PhD/Neoadjuvant/tum_lymph_overlap/count/' num2str(image_filenumber) '_centroidin_count20.csv'], t_l_centroid_count_cell);
%     
...
%     csvwrite(['/Users/cope01/Documents/OneDrive - University Of Cambridge/Documents/PhD/Neoadjuvant/tum_lymph_overlap/area/' num2str(image_filenumber) '_intersectionbuffer_area2.csv'], tbuffer_l_intersection_area_cell);
    csvwrite([output_dir '/outputfiles_tum_lymph/' num2str(image_filenumber) '/' num2str(image_filenumber) '_centroidbuffer_count20.csv'], tbuffer_l_centroid_count_cell);
    csvwrite([ '/Users/cope01/Documents/OneDrive - University Of Cambridge/Documents/PhD/Neoadjuvant/tum_lymph_overlap/count/' num2str(image_filenumber) '_centroidbuffer_count20.csv'], tbuffer_l_centroid_count_cell);
