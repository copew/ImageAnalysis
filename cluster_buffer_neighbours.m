...
% 2nd feb 2021 pair wised distance for every cell not only til 15
...
%%
%function cluster_buffer(image_filenumber)
for image = 1:size(image_list,2)
    ...
    %data = fitsread(['/Volumes/Wei/MatLab_folders_from_laptop/IT_PT_zone/' num2str(image_filenumber) '.fits'],'binarytable');
    %info = fitsinfo(['/Volumes/Wei/MatLab_folders_from_laptop/IT_PT_zone/' num2str(image_filenumber) '.fits']);
    %image_path = ['/Volumes/Wei/MatLab_folders_from_laptop/IT_PT_zone/' num2str(image_filenumber) '.svs'];
...
%% 
 % 
    ...
    for this_core = 1:size(core_polygon,2)
        ...
        %calculating distances of all neighbours
        %[all_multi_real_distances{this_core}, all_indexes{this_core}] = pdist2([in_core{this_core}{X_ind}(neighbour_cells) in_core{this_core}{Y_ind}(neighbour_cells)],[in_core{this_core}{X_ind}(base_cells) in_core{this_core}{Y_ind}(base_cells)],'euclidean');
...
%         end
        ...
        base_cells_all = in_core{this_core}{cell_ind}>0;
        neighbour_cells_all = in_core{this_core}{cell_ind}>0;
        [all_multi_real_distances_all{this_core}, all_indexes_all{this_core}] = pdist2([in_core{this_core}{X_ind}(neighbour_cells_all) in_core{this_core}{Y_ind}(neighbour_cells_all)],[in_core{this_core}{X_ind}(base_cells_all) in_core{this_core}{Y_ind}(base_cells_all)], 'euclidean','smallest', sum(in_core{this_core}{cell_ind}>0));
        
        for m=1:size(all_indexes_all{this_core}, 2)
            neighbourhood{this_core}(:,m) = in_core{this_core}{cell_ind}(all_indexes_all{this_core}(:,m));
        end
...
%     end
    ...
    %%
    %combine the neighbourhood info
    %neighbourhood_combined = cell(0);
    %neighbourhood_combined = horzcat(neighbourhood{:});
    ...
    %csvwrite([num2str(image_filenumber) '_neighbourhood.csv'], neighbourhood_combined);
    
    end
end
toc

%% Take Neighbourhood 3 as a large core - columns are cells, rows are neighbours  
total_tumour = sum(neighbourhood{3}(:,1)==1);
total_lymph = sum(neighbourhood{3}(:,1)==2);
total_stroma = sum(neighbourhood{3}(:,1)==3);

%Tumout Tumour first
is_tumour = neighbourhood{3}(1,:)==1;
all_tumours = neighbourhood{3}(:,is_tumour);

proportion_tumours_normalised = (sum(all_tumours==1,2)/size(all_tumours,2))*(size(neighbourhood{3},2)/total_tumour);
cumulative_tumours_normalised = cummean(proportion_tumours_normalised);
proportion_lymph_normalised = (sum(all_tumours==2,2)/size(all_tumours,2))*(size(neighbourhood{3},2)/total_lymph);
cumulative_lymph_normalised = cummean(proportion_lymph_normalised);
proportion_stroma_normalised = (sum(all_tumours==3,2)/size(all_tumours,2))*(size(neighbourhood{3},2)/total_stroma);
cumulative_stroma_normalised = cummean(proportion_stroma_normalised);

figure

hold on
plot(cumulative_tumours_normalised,'Color',[0.2,0.8,0.2],'LineWidth',3)
plot(cumulative_lymph_normalised,'r','LineWidth',3)
plot(cumulative_stroma_normalised,'b','LineWidth',3)
%plot([0, size(all_tumours,2)],[1 1],'k--','LineWidth',2)
plot([0, 10000],[1 1],'k--','LineWidth',2)
xlim([0, size(neighbourhood{3},2)])
ylim([0,3.5])
set(gca,'FontSize',16)
xlabel('Neighbour Number from Tumour Cell')
ylabel('Normalised Ratio')
set(gcf,'color','w')
legend({'Tumour','Lymphocyte','Stroma'})
saveas(gcf,'Example Single Core Tumour.png')