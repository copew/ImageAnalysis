...
for image = 1:size(image_list,2)
    ...
    load([output_dir '/outputfiles_tum_lymph/' num2str(image_filenumber) '/' num2str(image_filenumber) '_tbuffer_l_intersection.mat']);
    ...
    for i=1:size(tbuffer_l_intersection, 2) %looking at the cores with tumours
        %         tbuffer_l_intersection{i}=[];
        %         tbuffer_l_intersection_area{i}=[];
        tbuffer_l_intersection_lymph_count{i} = [];
        
        if isempty(tbuffer_l_intersection)
            continue
        end
        
        for j=1:size(tbuffer_l_intersection{i}, 2)
            
            if isempty(tbuffer_l_intersection{i}(j))
                continue
            end
            
            tmp = tbuffer_l_intersection{i}(j);
            if tmp.NumRegions == 0
                continue
            end
