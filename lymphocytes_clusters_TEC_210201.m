...
% loading files
data = fitsread(['../../Neoadjuvant/IT_PT_zone/' num2str(image_filenumber) '.fits'],'binarytable');
info = fitsinfo(['../../Neoadjuvant/IT_PT_zone/' num2str(image_filenumber) '.fits']);
image_path = ['../../Neoadjuvant/IT_PT_zone/' num2str(image_filenumber) '.svs'];
...
for i =1:size(core_list, 2)
    ...
    for k = 1:size(this_lymphocyte_cluster_boundary{core_list(i)}, 2)
        if isempty(this_lymphocyte_cluster_boundary{i}{k}) == 1
            continue
        end
...
% the above is unnecessary as only the buffer goes outside
    ...
    for k = 1:size(tumour_buffer{core_list(i)},2)
        ...
        tumour_buffer_in{i}{k} = intersect(tumour_buffer{i}{k}, core_polygon{core_list(i)});
    end
end
...
parfor i = 1:size(tumour_polygon, 2)
...
for i = 1:size(core_list, 2)
    ...
    for k = 1:size(tumour_buffer_in{i},2)
        tumour_buffer_area{i}{k} = area(tumour_buffer_in{i}{k});
    end
...
%             lymph_in_tumour{i}{j}{k} = data_trimmed{k}(lymph_in{i}{j}, :);
%         end
%         [lymph_in_distances{i}{j}, lymph_in_indexes{i}{j}] = pdist2([lymph_in_tumour{i}{j}{X_ind} lymph_in_tumour{i}{j}{Y_ind}],[lymph_in_tumour{i}{j}{X_ind} lymph_in_tumour{i}{j}{Y_ind}],'euclidean','Smallest', 51);
%     end
%     for m = 1:size(tumour_buffer_in{i}, 2)
%         for n = 1:size(data_trimmed, 2)
%             lymph_in_buffer{i}{m}{n} = data_trimmed{n}(lymph_buffer{i}{m}, :);
%         end
%         [lymph_buffer_distances{i}{m}, lymph_buffer_indexes{i}{m}] = pdist2([lymph_in_buffer{i}{m}{X_ind} lymph_in_buffer{i}{m}{Y_ind}],[lymph_in_buffer{i}{m}{X_ind} lymph_in_buffer{i}{m}{Y_ind}],'euclidean','Smallest', 51);
%     end
% end
% 
