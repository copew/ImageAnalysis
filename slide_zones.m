% A script that:
% 1. takes the .svs and .fits file
% 2. delineates the cores
% 3. delineates the tumour clusters
% 4. determines the overlap in order to find the actual area of interest
% 5. work out density of lymphocyte in the zones


%function zone_density(image_filenumber)
%%
% This chunk is for loading the .svs and .fits files and defining basic
% parameters;

% The cluster size is chosen after reviewing slides to allow for both IDC
% and ILC;
cluster_size = [5]; 

% Define the sizes of the detailed outputs - 50 is a lot
% clusters_for_detail = [15]; 

% Whether or not to check relative scaling of FITS and SVS
visualise = 0; 
...
%%
%this bit is to visualise the delineation
...
all_combinations = combvec(1:3,1:3); % 0:4 includes rubbish, 1:4 excludes, normal is then combined into tumour at a later step
% key{1} = 'rubbish';
% key{2} = 'tumour';
% key{3} = 'lymphocyte';
% key{4} = 'stroma';
