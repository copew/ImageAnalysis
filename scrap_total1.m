...
% hold on;
% for i = 1:size(core_polygon, 2)
%     plot(in_core{i}{X_ind}(in_core{i}{cell_ind}==1), in_core{i}{Y_ind}(in_core{i}{cell_ind}==1), 'b.') %again in case the
% 
            ...
            if ~visited(this_cell)
                ...
                if numel(Neighbours)<this_clustsize
                    ...
                    for k2 = 1:(k-1) %Competitive cluster size comparison step
                        ...
                        elseif sum(cluster_membership{i}(:) == temp_cluster_membership{i}(this_neighbour_cell)) < k
                            cluster_membership{i}(this_neighbour_cell)=C;
                        end
                    end
                end
